...
expansion_file = sys.argv[5]    # "expansion-file"
...
for query_id, query_text in Queries.items():
    corpus.append(preProcessText(query_text,lowercase=True,Stopwords=True,contractions=True,punctuations=True,digits=True,stemming=False).split())

for doc_id, doc_info in Docs.items():
    corpus.append(preProcessText((doc_info['title']+' '+doc_info['body']),lowercase=True,Stopwords=True,contractions=True,punctuations=True,digits=True,stemming=False).split())

print("Training Corpus Prepared")

# Train Word2Vec to generate local embeddings
print("Training Model ...")
w2v_model = Word2Vec(sentences=corpus, vector_size=300, window=5, min_count=5, sg=1)
print("Model Trained")
# Word2Vec parameters:
# - size: Dimensionality of the word vectors (typically 300)
# - window: Maximum distance between the current and predicted word within a sentence (typically 5)
# - min_count: Ignores all words with total frequency lower than this
# - sg: Skip-gram model (1) or CBOW model (0)
...
Top_N = 15                  # Number of Query Expansion Terms
...
for qid,qtext in Queries.items():   # query_id, query_text
    ...
    original_QT = SimpleTokenizer().tokenize(query)     # Original Query Terms
